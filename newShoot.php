<?php
    ...
    function createNewShoot($email, $account, $location, $date, $pdo) {
        try {
            ...
            $stmt_custID->execute([':accountID' => $accountID]);
            $row_custID = $stmt_custID->fetch(PDO::FETCH_ASSOC);
            $custID = $row_custID['custID'];
    
            // Find photographerID from Photographer table
            $stmt_photographerID = $pdo->prepare("SELECT photographerID FROM Photographer WHERE email = :email");
            $stmt_photographerID->execute([':email' => $email]);
            $row_photographerID = $stmt_photographerID->fetch(PDO::FETCH_ASSOC);
            $photographerID = $row_photographerID['photographerID'];
